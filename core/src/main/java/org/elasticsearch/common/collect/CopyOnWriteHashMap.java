...
public final class CopyOnWriteHashMap<K, V> extends AbstractMap<K, V> {
    ...
    private abstract static class Node<K, V> {
         ...
         */
        abstract void visit(Deque<Entry<K, V>> entries, Deque<Node<K, V>> nodes);
    ...
    private static class Leaf<K, V> extends Node<K, V> {
        ...
        @Override
        void visit(Deque<Entry<K, V>> entries, Deque<Node<K, V>> nodes) {
            for (int i = 0; i < keys.length; ++i) {
                entries.add(new SimpleImmutableEntry<>(keys[i], values[i]));
            }
        }
        ...
        void visit(Deque<Entry<K, V>> entries, Deque<Node<K, V>> nodes) {
            for (int i = 0; i < keys.length; ++i) {
                ...
                if (sub instanceof Node) {
                    ...
                    @SuppressWarnings("unchecked")
                    final V value = (V) sub;
                    entries.add(new SimpleImmutableEntry<>(keys[i], value));
                }
            }
        }
    ...
    private static class EntryIterator<K, V> implements Iterator<Entry<K, V>> {
        ...
        private final Deque<Entry<K, V>> entries;
