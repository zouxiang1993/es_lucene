...
class InternalOrder extends Order {
     ...
     */
    public static final InternalOrder COUNT_DESC = new InternalOrder(COUNT_DESC_ID, "_count", false, new Comparator<Bucket>() {
        @Override
     ...
     */
    public static final InternalOrder COUNT_ASC = new InternalOrder(COUNT_ASC_ID, "_count", true, new Comparator<Bucket>() {
     ...
     * Order by the terms.
     */
    public static final InternalOrder TERM_DESC = new InternalOrder(TERM_DESC_ID, "_term", false, new Comparator<Bucket>() {
     ...
     * Order by the terms.
     */
    public static final InternalOrder TERM_ASC = new InternalOrder(TERM_ASC_ID, "_term", true, new Comparator<Bucket>() {
    ...
    public static boolean isCountDesc(Order order) {
        if (order == COUNT_DESC) {
            return true;
    ...
    public static boolean isTermOrder(Order order) {
        if (order == TERM_ASC) {
            return true;
    ...
    InternalOrder(byte id, String key, boolean asc, Comparator<Bucket> comparator) {
        this.id = id;
        this.key = key;
        this.asc = asc;
    ...
    public static Order validate(Order order, Aggregator termsAggregator) {
        if (order instanceof CompoundOrder) {
            for (Order innerOrder : ((CompoundOrder)order).orderElements) {
    ...
    static class Aggregation extends InternalOrder {
        ...
        Aggregation(String key, boolean asc) {
            super(ID, key, asc, new MultiBucketsAggregation.Bucket.SubAggregationComparator<Bucket>(key, asc));
        }
        ...
        @Override
        protected Comparator<Bucket> comparator(Aggregator termsAggregator) {
    ...
    static class CompoundOrder extends Order {

        static final byte ID = -1;

        private final List<Order> orderElements;

        CompoundOrder(List<Order> compoundOrder) {
            this(compoundOrder, true);
        }

        CompoundOrder(List<Order> compoundOrder, boolean absoluteOrdering) {
            this.orderElements = new LinkedList<>(compoundOrder);
            Order lastElement = compoundOrder.get(compoundOrder.size() - 1);
        ...
        public static class CompoundOrderComparator implements Comparator<Bucket> {

            private List<Order> compoundOrder;
            private Aggregator aggregator;

            CompoundOrderComparator(List<Order> compoundOrder, Aggregator aggregator) {
            ...
            public int compare(Bucket o1, Bucket o2) {
                int result = 0;
                for (Iterator<Order> itr = compoundOrder.iterator(); itr.hasNext() && result == 0;) {
    ...
    public static class Streams {
        ...
        public static void writeOrder(Order order, StreamOutput out) throws IOException {
            if (order instanceof Aggregation) {
                ...
                CompoundOrder compoundOrder = (CompoundOrder) order;
                    ...
                    for (Order innerOrder : compoundOrder.orderElements) {
        ...
        public static Order readOrder(StreamInput in, boolean absoluteOrder) throws IOException {
            byte id = in.readByte();
            switch (id) {
                case COUNT_DESC_ID: return absoluteOrder ? new CompoundOrder(Collections.singletonList((Order) InternalOrder.COUNT_DESC)) : InternalOrder.COUNT_DESC;
                case COUNT_ASC_ID: return absoluteOrder ? new CompoundOrder(Collections.singletonList((Order) InternalOrder.COUNT_ASC)) : InternalOrder.COUNT_ASC;
                ...
                case CompoundOrder.ID:
                    ...
                    List<Order> compoundOrder = new ArrayList<>(size);
