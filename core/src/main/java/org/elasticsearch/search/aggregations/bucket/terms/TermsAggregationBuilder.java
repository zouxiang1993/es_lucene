...
public class TermsAggregationBuilder extends ValuesSourceAggregationBuilder<ValuesSource, TermsAggregationBuilder> {
    ...
    private Order order = Order.compound(Order.count(false), Order.term(true));
     ...
     */
    public TermsAggregationBuilder order(Order order) {
        if (order == null) {
        ...
        if(order instanceof CompoundOrder || InternalOrder.isTermOrder(order)) {
            ...
            this.order = Order.compound(order);
        }
        return this;
    }
     ...
     * Sets the order in which the buckets will be returned.
     */
    public TermsAggregationBuilder order(List<Order> orders) {
        if (orders == null) {
    ...
    private static Order parseOrderParam(XContentParser parser, QueryParseContext context) throws IOException {
    ...
    static Order resolveOrder(String key, boolean asc) {
        if ("_term".equals(key)) {
