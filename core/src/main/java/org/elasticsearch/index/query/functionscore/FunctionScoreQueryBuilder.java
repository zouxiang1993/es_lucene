...
public class FunctionScoreQueryBuilder extends AbstractQueryBuilder<FunctionScoreQueryBuilder> {
    ...
    public static Optional<FunctionScoreQueryBuilder> fromXContent(QueryParseContext parseContext) throws IOException {
                ...
                if (SCORE_MODE_FIELD.match(currentFieldName)) {
                    ...
                    if (WEIGHT_FIELD.match(currentFieldName)) {
                        filterFunctionBuilders.add(
                                new FilterFunctionBuilder(new WeightBuilder().setWeight(parser.floatValue())));
                        singleFunctionFound = true;
                        singleFunctionName = currentFieldName;
                    } else {
        ...
        FunctionScoreQueryBuilder functionScoreQueryBuilder = new FunctionScoreQueryBuilder(query,
                filterFunctionBuilders.toArray(new FilterFunctionBuilder[filterFunctionBuilders.size()]));
        if (combineFunction != null) {
    ...
    private static String parseFiltersAndFunctions(QueryParseContext parseContext,
            List<FilterFunctionBuilder> filterFunctionBuilders) throws IOException {
        String currentFieldName = null;
        XContentParser.Token token;
        XContentParser parser = parseContext.parser();
