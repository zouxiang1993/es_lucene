 ...
 * see {@link LegacyNumericUtils}). A range is then divided recursively into multiple intervals for searching:
...
public final class LegacyNumericRangeQuery<T extends Number> extends MultiTermQuery {
  ...
  /**
   * Factory that creates a <code>LegacyNumericRangeQuery</code>, that queries a <code>long</code>
   * range using the default <code>precisionStep</code> {@link LegacyNumericUtils#PRECISION_STEP_DEFAULT} (16).
   * You can have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries)
   * by setting the min or max value to <code>null</code>. By setting inclusive to false, it will
   * match all documents excluding the bounds, with inclusive on, the boundaries are hits, too.
   */
  ...
  /**
   * Factory that creates a <code>LegacyNumericRangeQuery</code>, that queries a <code>int</code>
   * range using the default <code>precisionStep</code> {@link LegacyNumericUtils#PRECISION_STEP_DEFAULT_32} (8).
   * You can have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries)
   * by setting the min or max value to <code>null</code>. By setting inclusive to false, it will
   * match all documents excluding the bounds, with inclusive on, the boundaries are hits, too.
   */
   ...
   * {@link Term#compareTo}.
   * The ordering depends on how {@link LegacyNumericUtils#splitLongRange} and
   * {@link LegacyNumericUtils#splitIntRange} generates the sub-ranges. For
